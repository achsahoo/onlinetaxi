...
//	area in dropdown out station
	...
	//json all data in bookingdetails table
	@ResponseBody
	@GetMapping("/bookingdetails")
	public ResponseEntity<Iterable<BookingDetails>> bookingdetailsjson()
	{
		ResponseEntity<Iterable<BookingDetails>> ob =null ;
		Iterable<BookingDetails> boodetails= taxiservice.showBooking();
		ob= new ResponseEntity<>(boodetails,HttpStatus.OK);
		return ob;
	}
	
	//json area inside bbsr
	@ResponseBody
	@GetMapping("/area1")
	public ResponseEntity<List<VehicleAreaPrice>> findTyreAreajson(){
		ResponseEntity<List<VehicleAreaPrice>> res=null;
		List<VehicleAreaPrice> list=taxiservice.showArea();
		res=new ResponseEntity<>(list,HttpStatus.OK);
		return res;
	}
	
//	area in dropdown out station json
	@ResponseBody
	@GetMapping("/outarea1")
	public ResponseEntity<List<OutstationDetails>> findTyreArea1json(){
		List<OutstationDetails> listdto=null;
		ResponseEntity<List<OutstationDetails>> ob=null;
		listdto=outstationService.showArea();
		ob=new ResponseEntity<>(listdto,HttpStatus.OK);
		return ob;
